    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between gap-3 mb-6">
        <div className="page-header !mb-0">
          ...
          <p className="page-sub">Create and manage discount codes</p>
        </div>
        <button onClick={() => setShowForm(!showForm)} className={`${showForm ? 'btn-secondary' : 'btn-gold'} !py-2.5 !px-5 inline-flex items-center gap-1.5 text-sm shrink-0`}>
      ...
      {/* Create Form */}
      {showForm && (
        <form onSubmit={handleCreate} className="panel p-4 md:p-5 mb-6 space-y-4 animate-fade-in">
          <h3 className="text-sm font-semibold text-white font-display">New Coupon</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Code *</label>
              <input type="text" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} className="w-full input-field font-mono" placeholder="SAVE20" required />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Discount Type</label>
              <select value={form.discountType} onChange={(e) => setForm({ ...form, discountType: e.target.value })} className="w-full select-field">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" value={form.discountValue} onChange={(e) => setForm({ ...form, discountValue: e.target.value })} className="w-full input-field pl-8" min="1" required />
              </div>
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Min Amount (₹)</label>
              <input type="number" value={form.minAmount} onChange={(e) => setForm({ ...form, minAmount: e.target.value })} className="w-full input-field" placeholder="0" min="0" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Max Uses</label>
              <input type="number" value={form.maxUses} onChange={(e) => setForm({ ...form, maxUses: e.target.value })} className="w-full input-field" placeholder="Unlimited" min="1" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Expires At</label>
              <input type="date" value={form.expiresAt} onChange={(e) => setForm({ ...form, expiresAt: e.target.value })} className="w-full input-field" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Applicable Mod</label>
              <select value={form.applicableModId} onChange={(e) => setForm({ ...form, applicableModId: e.target.value })} className="w-full select-field">
            ...
            <div>
              ...
              <input type="text" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full input-field" placeholder="Optional" />
            </div>
          </div>
          <button type="submit" disabled={saving} className="btn-gold !py-2 !px-6 text-sm disabled:opacity-50">
      ...
      {coupons.length === 0 ? (
        ...
        <div className="grid gap-3">
          {coupons.map((c) => {
            ...
            return (
              <div key={c._id} className={`panel p-4 md:p-5 transition-all ${isInvalid ? 'border-red-500/20 opacity-60' : ''}`}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      isExpired ? 'chip-red' :
                      isExhausted ? 'chip-yellow' :
                      c.isActive ? 'chip-green' : 'chip-gray'
                    }`}>
