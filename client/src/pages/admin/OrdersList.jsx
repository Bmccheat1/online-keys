...
export default function OrdersList() {
  ...
  return (
    <div>
      <h1 className="text-xl md:text-2xl font-bold mb-6 text-white">Orders</h1>
        ...
        <div className="bg-[#0d0d1a]/80 backdrop-blur-sm border border-[#1e1e2e]/60 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px]">
              ...
              <tbody>
                {orders.map((order) => (
                  <tr key={order._id} className="border-b border-[#1e1e2e]/30 hover:bg-[#0a0a14]/50 transition-colors">
                    ...
                    <td className="p-3 md:p-4">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${
                        order.paymentStatus === 'completed' ? 'bg-emerald-500/10 text-emerald-400' :
